...
from robocandywrapper.datasets.legacy_utils import (
    append_jsonlines,
...
class LegacyLeRobotDatasetMetadata:
    ...
    def create(
        ...
        """Creates metadata for a LegacyLeRobotDataset."""
        obj = cls.__new__(cls)
        obj.repo_id = repo_id
...
class LegacyLeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        repo_id: str,
        root: str | Path | None = None,
        ...
        2. Your dataset doesn't already exists (either on local disk or on the Hub): you can create an empty
           LegacyLeRobotDataset with the 'create' classmethod. This can be used for recording a dataset or port an
           existing dataset to the LegacyLeRobotDataset format.


        In terms of files, LegacyLeRobotDataset encapsulates 3 main things:
            - metadata:
        ...
        timestamps = torch.stack([torch.tensor(t) if not isinstance(t, torch.Tensor) else t 
                                   for t in self.hf_dataset["timestamp"]]).numpy()
    ...
    def create(
        ...
        video_backend: str | None = None,
        batch_encoding_size: int = 1,
    ) -> "LegacyLeRobotDataset":
        """Create a Legacy LeRobot Dataset from scratch in order to record data."""
        obj = cls.__new__(cls)
        obj.meta = LegacyLeRobotDatasetMetadata.create(
            repo_id=repo_id,
            fps=fps,
...
class LegacyMultiLeRobotDataset(torch.utils.data.Dataset):
    """A dataset consisting of multiple underlying `LegacyLeRobotDataset`s.

    The underlying `LegacyLeRobotDataset`s are effectively concatenated, and this class adopts much of the API
    structure of `LegacyLeRobotDataset`.
    """
